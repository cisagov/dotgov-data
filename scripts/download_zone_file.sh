...
curl -X GET --output gov.zone.txt.gz -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: Bearer $token" --compressed https://czds-api.icann.org/czds/downloads/gov.zone